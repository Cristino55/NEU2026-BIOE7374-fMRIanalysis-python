...
func_file = data_dir+'sub-02/ses-test/func/sub-02_ses-test_task-fingerfootlips_bold.nii.gz'
...
# from nipype.interfaces.fsl import ExtractROI
# extract=ExtractROI(in_file=data_dir+'sub-02/ses-test/func/sub-02_ses-test_task-fingerfootlips_bold.nii.gz',
...
# Specify example input file
anat_file = data_dir+'sub-02/ses-test/anat/sub-02_ses-test_T1w.nii.gz'
...
# Connect smoothing node to coregistration node above here
preproc.connect([(applywarp, smooth, [('out_file', 'in_files')])])

# old smoothing via SUSAN
...
# old: Connect gray matter Mask node to the susan workflow here
...
mask_func = MapNode(ApplyMask(output_type='NIFTI'),
                    name="mask_func",
                    iterfield=["in_file"])
# Connect smoothed output file to ApplyMask node here
preproc.connect([(smooth, mask_func, [('smoothed_files', 'in_file')]),
                 (mask_GM, mask_func, [('out_file', 'mask_file')])
                 ])

# old:  Connect smoothed susan output file to ApplyMask node here
...
# select subject 7
subject_list = ['02', '04', '07', '08'] # ['02','04','07','08']
...
preproc.write_graph(graph2use='orig', format='png', simple_form=False)
