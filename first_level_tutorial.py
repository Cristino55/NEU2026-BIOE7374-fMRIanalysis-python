...
results_dir = course_dir+'/students/cristino.m/output/'
subject_list = ['02','04','07', '08']
...
### Special setup for Matlab, SPM, and FSL (extremely special setup for SPM...)
from nipype.interfaces.matlab import MatlabCommand
MatlabCommand.set_default_matlab_cmd(matlab_path) 
...
## Unzipping / renaming for SPM - TODO: quick fix for now; use nipype modules in future

import os
import subprocess

base_dir = results_dir+"work_preproc"
for subject_dir in os.listdir(base_dir):
    if subject_dir.startswith("_subject_id_"):
        subject_num = subject_dir.split("_")[-1]
        base_name = f"{subject_dir}/mcflirt/asub-{subject_num}_ses-test_task-fingerfootlips_bold_roi_mcf"
        if os.path.exists(f"{base_dir}/{base_name}.nii.gz"):
            print(f"Renaming / unzipping {base_dir}/{base_name}*")
            subprocess.run(["gunzip", f"{base_dir}/{base_name}.nii.gz"])
            subprocess.run(["mv", f"{base_dir}/{base_name}.nii.gz_mean_reg.nii.gz", f"{base_dir}/{base_name}.nii_mean_reg.nii.gz"])
            subprocess.run(["gunzip", f"{base_dir}/{base_name}.nii_mean_reg.nii.gz"])
            subprocess.run(["mv", f"{base_dir}/{base_name}.nii.gz.par", f"{base_dir}/{base_name}.nii.par"])
#import sys
#sys.exit()
...
# set up iteration over subject list
